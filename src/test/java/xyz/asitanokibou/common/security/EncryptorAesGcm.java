...
public class EncryptorAesGcm {
    ...
    public static byte[] encrypt(byte[] pText, byte[] rawEncryptionKey, byte[] iv/*, byte[] associatedData*/) throws Exception {
        return encrypt(pText, new SecretKeySpec(rawEncryptionKey, "AES"), iv);
    }

    // AES-GCM needs GCMParameterSpec
    public static byte[] encrypt(byte[] pText, SecretKey secret, byte[] iv/*, byte[] associatedData*/) throws Exception {

        Cipher cipher = Cipher.getInstance(ENCRYPT_ALGO);
        cipher.init(Cipher.ENCRYPT_MODE, secret, new GCMParameterSpec(AUTH_TAG_LENGTH_BIT, iv));
    ...
    public static String decrypt(byte[] cText, SecretKey secret, byte[] iv) throws Exception {
        ...
        Cipher cipher = Cipher.getInstance(ENCRYPT_ALGO);
        cipher.init(Cipher.DECRYPT_MODE, secret, new GCMParameterSpec(AUTH_TAG_LENGTH_BIT, iv));
